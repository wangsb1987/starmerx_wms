...
function save_click()
{
    ...
    $.ajax({
        type: "GET",
        url: '/starmerx_cggl/create_purchase_order',
        //data: {'orderline':a},
        data:{'orderline':JSON.stringify(a),'partner_id':$("#partner_id").val(),
...
function checkedit(){
    ...
    var selproduct_id=a[$("#selrowindex").val()].product_id;
    //===一般select设置选中项
    ...
    $("#price_unit").val(a[$("#selrowindex").val()].price_unit);
    $("#product_qty").val(a[$("#selrowindex").val()].product_qty);
    ...
    //$("#last_purchase_price").val($('#product_id').children('option:selected').attr("data-price"));
	$('#myModal').modal('show');
	$('#savebtn').hide();
	$('#editbtn').show();
}
...
$('#para_table tr').each(function(rowindex){
    if(rowindex!=0)
    {
        a.push({'product_id':$(this).find('td').eq(0).find("input").val(),'price_unit':$(this).find('td').eq(2).text(),
            'product_qty':parseInt($(this).find('td').eq(1).text())})
    }
});
}
